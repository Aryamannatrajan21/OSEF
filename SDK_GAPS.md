...
| `osef_python` | `EKGBuilder` fires no events | `EKGBuilder` orchestrates the build process but does not instantiate or use the `EventBus`. | The test suite will manually trigger the plugin parser alongside the core builder. | RESOLVED - Migrated to `PipelineEngine` and `EventBus` hooks |
