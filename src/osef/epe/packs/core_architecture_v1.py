...
from osef.epe.core.result import (
    RuleResult, Finding, Provenance, Evidence, Recommendation, Severity, Category, AutoFix
)
...
class MissingDocumentationRule(Rule):
    ...
    def evaluate(self, context: RuleContext) -> RuleResult:
        findings = []
        funcs = context.get_nodes_by_type("function") + context.get_nodes_by_type("method")
        ...
        for node in funcs + classes:
            if not node.description:
                findings.append(Finding(
                    ...
                    confidence=1.0,
                    provenance=Provenance(rule_id=self.id, rule_version=self.version, policy_version=context.policy_version),
                    evidence=Evidence(description="Node description is empty.", affected_nodes=[node.id]),
                    recommendation=Recommendation(action="Add docstring", description=f"Add a PEP-257 compliant docstring to {node.name}."),
        ...
        for node in context.get_nodes_by_type("import"):
            if node.metadata.get("resolved") != "true":
                findings.append(Finding(
                    ...
                    confidence=1.0,
                    provenance=Provenance(rule_id=self.id, rule_version=self.version, policy_version=context.policy_version),
                    evidence=Evidence(description="Import node lacks 'resolved=true' metadata.", affected_nodes=[node.id]),
                    recommendation=Recommendation(action="Verify dependency", description="Ensure the module is installed or path is correct.")
                ))
        return RuleResult(findings=findings)
