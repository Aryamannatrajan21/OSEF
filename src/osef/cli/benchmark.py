...
def list() -> None:
    ...
    for manifest in registry.manifests:
        console.print(
            f"- [green]{manifest.name}[/green] ({manifest.tier}): {manifest.repository}"
        )
...
def info(benchmark: str) -> None:
    ...
    console.print(
        f"  [bold]Expected Nodes:[/bold] {manifest.expected.get('minimum_nodes')}"
    )
    console.print(
        f"  [bold]Expected Edges:[/bold] {manifest.expected.get('minimum_edges')}"
    )
    console.print(
        f"  [bold]Expected Confidence:[/bold] {manifest.expected.get('engineering_confidence')}"
    )
