...
class CrossLanguageCertificationEngine:
    ...
    def _check_abstraction_leakage(self, obj: Any) -> AbstractionLeakageReport:
        ...
        try:
            ...
            if leaks:
                return AbstractionLeakageReport(
                    has_leakage=True,
                    leaked_artifacts=leaks,
                    violation_message="Found parser-specific tokens",
                )
        except Exception:
            pass
        return AbstractionLeakageReport(
            has_leakage=False, leaked_artifacts=[], violation_message="Clean"
        )

    def certify_fixture(
        self, fixture_dir: str, matrix_path: str
    ) -> CrossLanguageCertificationReport:
        ...
        for lang, pipeline in self.pipelines.items():
            source_file = (
                f"{fixture_dir}/{lang}/source.ts"  # Fallback extension for TS mock
            )
            if lang == "typescript":
