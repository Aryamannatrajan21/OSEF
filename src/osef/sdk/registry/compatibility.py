...
class CompatibilityEngine:
    ...
    def check_plugin(
        self, manifest: PluginManifest, active_profile: Optional[str] = None
    ) -> bool:
        """
        Validates if a plugin can run in the current environment.
        """
        if manifest.sdk_version != self.sdk_version and not self.sdk_version.startswith(
            "0.1."
        ):
        ...
        if (
            manifest.graph_schema != self.graph_schema
            and manifest.graph_schema != "any"
        ):
            raise OSEFError(
                f"Plugin {manifest.name} requires graph schema {manifest.graph_schema} but system is {self.graph_schema}"
            )

        if active_profile and manifest.supported_profiles:
            if active_profile not in manifest.supported_profiles:
                raise OSEFError(
                    f"Plugin {manifest.name} does not support profile {active_profile}"
                )
