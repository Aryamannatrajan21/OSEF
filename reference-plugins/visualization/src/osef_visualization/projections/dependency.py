...
from osef_visualization.ir import ProjectedGraph, ProjectionMetadata, ProjectedNode, ProjectedEdge
    ...
    def project(self, query: GraphQuery, **kwargs: Any) -> ProjectedGraph:
        ...
        for edge in all_dependency_edges:
            edges.append(ProjectedEdge(
                ...
                attributes={"style": "dashed" if edge.relation_type == "imports" else "solid"}
            ))
        ...
        return ProjectedGraph(metadata=metadata, nodes=list(nodes_dict.values()), edges=edges)
