...
def test_determinism(parser):
    ...
    assert len(hashes) == 1, "Determinism failed! Multiple hashes produced across 100 runs."
...
def test_hash_stability(parser, tmp_path):
    ...
    def structural_hash(node: NormalizedASTNode) -> str:
        ...
        struct = {"kind": node.kind, "children": [structural_hash(c) for c in node.children]}
        return hashlib.sha256(json.dumps(struct, sort_keys=True).encode()).hexdigest()
        
    assert structural_hash(ast_original) == structural_hash(ast_whitespace), "Structural Hash Stability failed!"
