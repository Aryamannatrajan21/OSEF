...
def test_semantic_engine():
    parsing_prov = ParsingProvenance(
        ...
        source_file="test.ts", source_hash="abcd", ast_node_kind="class_declaration",
        source_range=[1,1,2,2]
    )
    semantic_prov = SemanticProvenance(
        semantic_stage="symbol_extraction", resolver_version="N/A", plugin_version="0.1",
        sdk_version="0.1", graph_schema_version="1.0", normalized_symbol_id="test_id"
    )
    class_symbol = NormalizedClass(
        symbol_id="typescript::test.ts::UserService::class", name="UserService",
        parsing_provenance=parsing_prov, semantic_provenance=semantic_prov,
        modifiers=["public"]
    )
